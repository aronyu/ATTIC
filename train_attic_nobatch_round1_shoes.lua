-------------------------------------------------------------------------------
--
-- Active Training Image Creation (Round 1)
--
-- Description:
...
--------------------------
opts1 = {}
opts1.model_dir = 'models/pretrained/zap50k_attr2img/'  -- MOD: path to pretained image generator model
...
opts2.trainDataReal = 'train-zap50k-' .. opts2.attrName .. '.h5'   -- real training pairs
...
-- MOD: Adjust according to your own file structure
...
num_real_train = torch.ceil(num_real_train_full * real_base_fact)
...
for epoch = 1, opts2.numIter do
  ...
  if ( (save_model == 1) and (math.fmod(epoch, save_model_gap) == 0) ) then
    ...
    local snapshot_params = base_dir .. 'nobatch_part1_snapshot_params.dat'
    torch.save(snapshot_params, snapshot)
    print(string.format('\nParameters Saved @ Epoch %d', epoch))
    
    -- Save Individual Models
    save_abatch_models(1, epoch, base_dir, netPreGen, netGen, netRank)
...
local fsynth_name = base_dir .. 'abatch1_fsynth_data.h5'
